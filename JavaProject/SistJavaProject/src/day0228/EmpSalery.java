...
class Emp{
	...
	//오버타임수당..시간당 근무수당 리턴=초과시간당 3만원(단 10시간 이상은 무조건 30만원으로 통일)
	...
	{
		int tp=pay+getFamSudang()+getTimeSudang(); //pay=getPay()
		return tp;
	}
	//제목을 메서드로
...
/////////////////////
//메인에서는 인원을 입력후 해당 사원에 대한 이름, 기본급, 가족수, 오버타임을 입력받아 출력할것!!!
public class EmpSalery {
	//emp클래스 결과출력
	public static void showEmpSalery(Emp[] emp) //Emp: 클래스명 emp:인스턴스 변수명
	{
		Emp.showTitle();
		//for~each문 
		for(Emp e:emp)
		...
		{
			System.out.println(emp[i].getName()+"\t"+emp[i].getPay()+"\t"+emp[i].getFamSu()+"\t"
		+emp[i].getTimeSu()+"\t"+emp[i].getFamSudang()+"\t"+emp[i].getTimeSudang()+"\t\t"+emp[i].getTotalPay());
		}*/
	}
