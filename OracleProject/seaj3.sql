...
--표현식 (컬럼이외에 출력을 원하는 내용을 select 구문뒤에 ' ' 사용한다)
...
--emp테이블에서 모든열을 조회하되 급여가 낮은 사람부터 조회하시오
...
select position "교수명", pay "급여", bonus "보너스" from professor order by pay desc;
--고객테이블에서 고객명,포인트를 조회하되 포인트가 높은사람부터 조회하시오
select gname "고객명", point "포인트" from gogak order by point desc;